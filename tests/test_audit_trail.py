...
@pytest.fixture
def auditor(db):
    user = get_user_model().objects.create_user("auditor", password="audit-pass-1")
...
@pytest.fixture
def manager(db):
    user = get_user_model().objects.create_user("manager2", password="audit-pass-2")
...
def test_a_user_without_the_permission_cannot_read_it(client, db):
    nobody = get_user_model().objects.create_user("nosy", password="audit-pass-3")
